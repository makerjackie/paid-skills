...
22. Event timing must not change the race animation. The year/value progress should stay continuous and smooth from start to end. Rank switching is a separate animation and should complete quickly at a fixed pace, around 0.8 seconds for each timeline step, instead of being stretched across the full year segment. Event metadata only controls what appears near the year or under an entity value; it must never make bars pause, speed up, or slow down.
